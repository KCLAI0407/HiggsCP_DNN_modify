...
def preprocess_data(args):
    ...
    if not reuse_weights or not os.path.exists(os.path.join(data_path, str(num_classes)+'weights.npy')) \
        ...
        np.save(os.path.join(data_path, str(num_classes+1)+'argmaxs.npy'), argmaxs)
        np.save(os.path.join(data_path, str(num_classes)+'hits_argmaxs.npy'), hits_argmaxs)
    weights  = np.load(os.path.join(data_path, str(num_classes+1)+'weights.npy'))
    argmaxs = np.load(os.path.join(data_path, str(num_classes+1)+'argmaxs.npy'))
